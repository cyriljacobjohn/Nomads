...
struct StylistViewProfile: View {
    ...
    @State private var clientId: Int = 15
    ...
    var body: some View {
        ZStack(alignment: .bottomLeading) {
            ...
            Button(action: {
                            self.presentationMode.wrappedValue.dismiss() // Dismisses the current view
                        }) {
                            Image(systemName: "arrow.left")
                                .foregroundColor(.white)
                                .padding()
                                .background(Color("PrimaryColor").opacity(0.5)) // Semi-transparent black background
                                .clipShape(Circle())
                                .shadow(radius: 4)
                                .padding()
                        }
